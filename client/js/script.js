      ...
      if (menuBlock.is(":hidden")) {
        if (params.onlyHide != true) {
          ...
          if (params.withOutAnimation == true) {
            ...
            menuBlock.animate({ marginLeft: "0" }, params.time);
          }
        }
      } else {
  ...
  showPopup: (param, element) => {
    ...
    if (param) {
      ...
      main_element
        ...
        .val(product_element.parents(".product-parent").data("id"));
...
$(window).resize(function () {
  ...
  if (windowWidth > 768)
    wideMenu.showHideMobile({ onlyShow: true, withOutAnimation: true });
  if (windowWidth <= 768)
    wideMenu.showHideMobile({ onlyHide: true, withOutAnimation: true });
});

$(document)
  .on("click", ".js_mobile_menu_open_button", function () {
    wideMenu.showHideMobile();
  })
  .on("click", ".js_close_mobile_menu", function () {
    wideMenu.showHideMobile();
  })
  .on("click", ".js_buy", function () {
    happyNewYear.showPopup(true, this);
  })
  .on("click", ".js_close-popup", function () {
    happyNewYear.showPopup(false);
  })
  .on("click", ".js_overlay", function () {
    happyNewYear.showPopup(false);
  })
  .on("click", ".js_send", function () {
    happyNewYear.sendOrder();
  });
